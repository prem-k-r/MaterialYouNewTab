...
async function getWeatherData() {
  ...
  async function fetchLocationSuggestions(query) {
    ...
    try {
      const response = await fetch(
        `https://api.weatherapi.com/v1/search.json?key=${savedApiKey}&q=${query}`
      );
  ...
  function updateActiveSuggestion() {
    const items = locationSuggestions.querySelectorAll(
  ...
  document.addEventListener("click", (e) => {
    if (
      ...
      !userLocInput.contains(e.target)
    ) {
      locationSuggestions.style.display = "none";
      suggestions = [];
      toggleAutocomplete();
  ...
  async function fetchWeather() {
    try {
      ...
      const weatherParsedTime = parseInt(
        localStorage.getItem("weatherParsedTime")
      );
      const weatherParsedLocation = localStorage.getItem(
        "weatherParsedLocation"
      );
        ...
        if (!parsedData.error) {
          ...
          const filteredData = {
            ...
            forecast: {
              ...
              forecastHours: parsedData.forecast.forecastday[0].hour.map(
                (e) => ({
                  time: e.time,
                  temp_c: e.temp_c,
                  temp_f: e.temp_f,
      ...
      // find data weather forecast hour wise
      function getForecastForNow() {
        const currentHour = new Date().getHours(); // 0–23

        return parsedData.forecast.forecastHours.find((item) => {
      ...
      if (!navigator.onLine) {
        ...
        const localizedMinTempC = localizeNumbers(
          minTempC.toString(),
          currentLanguage
        );
        const localizedMaxTempC = localizeNumbers(
          maxTempC.toString(),
          currentLanguage
        );
        const localizedMinTempF = localizeNumbers(
          minTempF.toString(),
          currentLanguage
        );
        const localizedMaxTempF = localizeNumbers(
          maxTempF.toString(),
          currentLanguage
        );
        ...
        const fahrenheitCheckbox =
          document.getElementById("fahrenheitCheckbox");
        const updateTemperatureDisplay = () => {
          ...
          const separator =
            rangeSeparator[currentLanguage] || rangeSeparator.default;
          ...
          if (fahrenheitCheckbox.checked) {
            ...
            const feelsLikeFUnit = langWithSpaceBeforeDegree.includes(
              currentLanguage
            )
              ? " °F"
              : "°F";
            ...
            // Update feels like or Min-Max temp
            const feelsLikeCUnit = langWithSpaceBeforeDegree.includes(
              currentLanguage
            )
              ? " °C"
              : "°C";
        ...
        const weatherIcon = newWIcon.replace(
          "//cdn.weatherapi.com/weather/64x64/",
          "https://cdn.weatherapi.com/weather/128x128/"
        );
        ...
        let maxLength = 10;
        let isLocationHidden =
          localStorage.getItem("locationHidden") === "true";
        ...
        function updateLocationText() {
          if (isLocationHidden) {
            ...
            const limitedText =
              city.length > maxLength ? city.slice(0, maxLength) + "..." : city;
        ...
        function getToggleIcon() {
          return isLocationHidden
            ? "./svgs/location-show.svg"
            : "./svgs/location-hide.svg";
        }
